...
func verifyState(ctx *cli.Context) error {
	...
	snaptree, err := snapshot.New(chaindb, trie.NewDatabase(chaindb, &trie.Config{ReadOnly: true}), 256, headBlock.Root(), false, false, false)
	if err != nil {
...
func traverseRawState(ctx *cli.Context) error {
	...
	for accIter.Next(true) {
		...
		if node != (common.Hash{}) {
			...
			// have their own hash).
			blob, _ := layer.NodeBlob(trie.EncodeInternalKey(accIter.StorageKey(), node))
			if len(blob) == 0 {
		...
		if accIter.Leaf() {
			...
			if acc.Root != emptyRoot {
				storageTrie, err := trie.NewSecureWithOwner(root, common.BytesToHash(accIter.LeafKey()), acc.Root, triedb)
				if err != nil {
					log.Error("Failed to open storage trie", "root", acc.Root, "err", err)
				...
				for storageIter.Next(true) {
					...
					// have their own hash).
					if node != (common.Hash{}) {
						blob, _ := layer.NodeBlob(trie.EncodeInternalKey(storageIter.StorageKey(), node))
						if len(blob) == 0 {
...
func dumpState(ctx *cli.Context) error {
	...
	snaptree, err := snapshot.New(db, trie.NewDatabase(db, &trie.Config{ReadOnly: true}), 256, root, false, false, false)
	if err != nil {
		return err
	}
