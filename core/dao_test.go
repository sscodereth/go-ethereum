...
func TestDAOForkRangeExtradata(t *testing.T) {
	...
	copyDB(db, gendb)
	prefix, _ := GenerateChain(params.TestChainConfig, genesis, ethash.NewFaker(), gendb, int(forkBlock.Int64()-1), func(i int, gen *BlockGen) {})
	...
	for i := int64(0); i < params.DAOForkExtraRange.Int64(); i++ {
		...
		copyDB(db, gendb1)
		copyDB(db, gendb2)
		blocks, _ = GenerateChain(&proConf, conBc.CurrentBlock(), ethash.NewFaker(), gendb1, 1, func(i int, gen *BlockGen) {})
		if _, err := conBc.InsertChain(blocks); err == nil {
			t.Fatalf("contra-fork chain accepted pro-fork block: %v", blocks[0])
		}
		// Create a proper no-fork block for the contra-forker
		blocks, _ = GenerateChain(&conConf, conBc.CurrentBlock(), ethash.NewFaker(), gendb2, 1, func(i int, gen *BlockGen) {})
		if _, err := conBc.InsertChain(blocks); err != nil {
		...
		copyDB(db, gendb1)
		copyDB(db, gendb2)
		blocks, _ = GenerateChain(&conConf, proBc.CurrentBlock(), ethash.NewFaker(), gendb1, 1, func(i int, gen *BlockGen) {})
		if _, err := proBc.InsertChain(blocks); err == nil {
			t.Fatalf("pro-fork chain accepted contra-fork block: %v", blocks[0])
		}
		// Create a proper pro-fork block for the pro-forker
		blocks, _ = GenerateChain(&proConf, proBc.CurrentBlock(), ethash.NewFaker(), gendb2, 1, func(i int, gen *BlockGen) {})
		if _, err := proBc.InsertChain(blocks); err != nil {
