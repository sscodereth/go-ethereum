...
func (bc *BlockChain) SetHeadBeyondRoot(head uint64, root common.Hash) (uint64, error) {
	...
	delFn := func(db ethdb.KeyValueWriter, hash common.Hash, num uint64) {
		...
		if num+1 <= frozen {
			...
			if err := bc.db.TruncateAncients(rawdb.ChainFreezer, num); err != nil {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		if err := batch.Write(); err != nil {
			...
			if err := bc.db.TruncateAncients(rawdb.ChainFreezer, fastBlock+1); err != nil {
		...
		if !updateHead(blockChain[len(blockChain)-1]) {
			...
			if err := bc.db.TruncateAncients(rawdb.ChainFreezer, previousFastBlock+1); err != nil {
				log.Error("Can't truncate ancient store after failed insert", "err", err)
			}
...
func (bc *BlockChain) insertSideChain(block *types.Block, it *insertIterator) (int, error) {
	...
	// Check if the state is present or the state is able to be recovered.
	parent := it.previous()
	for parent != nil && !bc.StateRecoverable(parent.Root) {
