...
func TestUpdateLeaks(t *testing.T) {
	...
	state, _ := New(common.Hash{}, 0, NewDatabase(db), nil)
...
func TestIntermediateLeaks(t *testing.T) {
	...
	transState, _ := New(common.Hash{}, 0, NewDatabase(transDb), nil)
	finalState, _ := New(common.Hash{}, 0, NewDatabase(finalDb), nil)
...
func TestCopy(t *testing.T) {
	...
	orig, _ := New(common.Hash{}, 0, NewDatabase(rawdb.NewMemoryDatabase()), nil)
...
func (test *snapshotTest) run() bool {
	...
	var (
		state, _     = New(common.Hash{}, 0, NewDatabase(rawdb.NewMemoryDatabase()), nil)
	...
	for sindex--; sindex >= 0; sindex-- {
		checkstate, _ := New(common.Hash{}, 0, state.Database(), nil)
...
func TestTouchDelete(t *testing.T) {
	...
	s.state, _ = New(root, 0, s.state.db, s.state.snaps)
...
func TestDeleteCreateRevert(t *testing.T) {
	...
	state, _ = New(root, 0, state.db, state.snaps)
