...
type testHelper struct {
	...
	// accumulated uncommitted dirty trie nodes
	uncommitted []*trie.CommitResult
}
...
func (t *testHelper) makeStorageTrie(stateRoot, owner common.Hash, keys []string, vals []string, commit bool) []byte {
	stTrie, _ := trie.NewSecureWithOwner(stateRoot, owner, common.Hash{}, t.triedb)
	for i, k := range keys {
	...
	if !commit {
		...
		root = result.Root
		t.uncommitted = append(t.uncommitted, result)
	}
	return root.Bytes()
}
...
func (t *testHelper) CommitAndGenerate(runGen bool) (*trie.CommitResult, *diskLayer) {
	...
	t.triedb.Update(result.Root, common.Hash{}, 0, result.CommitTo(nil))
...
func TestGenerateCorruptAccountTrie(t *testing.T) {
	...
	helper.triedb.Update(common.Hash{}, result.Root, 0, result.CommitTo(nil))
...
func TestGenerateMissingStorageTrie(t *testing.T) {
	...
	stRoot := helper.makeStorageTrie(common.Hash{}, accOneHash, []string{"key-1", "key-2", "key-3"}, []string{"val-1", "val-2", "val-3"}, true)
	...
	rawdb.DeleteTrieNode(helper.diskdb, trie.EncodeStorageKey(accThreeHash, nil))
