...
// If no state is locally available for the given block, recover the state
// by an ephemeral shadowy state database. The optional base statedb can be
// passed then it's regarded as the statedb of the parent block.
//
// Parameters:
...
func (eth *Ethereum) stateAtBlock(block *types.Block, parent *state.StateDB) (*state.StateDB, error) {
...
func (eth *Ethereum) stateAtTransaction(block *types.Block, txIndex int) (core.Message, vm.BlockContext, *state.StateDB, error) {
	...
	statedb, err := eth.stateAtBlock(parent, nil)
	if err != nil {
